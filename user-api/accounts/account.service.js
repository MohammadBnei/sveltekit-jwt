...
async function register(params, origin) {
    ...
    if (await db.Account.findOne({ email: params.email })) {
        ...
        throw new Error('There already is an account with this email : ' + params.email)
...
function generateJwtToken(account) {
    // create a jwt token containing the account id that expires in 30 minutes
    return jwt.sign({ sub: account.id, id: account.id }, process.env.SECRET, { expiresIn: '30m' });
}
